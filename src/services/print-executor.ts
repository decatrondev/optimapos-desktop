 ...
 *
 * Flow: PrintJob → renderPrintJob (text) → printerPrintTextTCP/USB (ESC/POS binary) → ack
 */

import { PrintJob } from '../types/printer-config';
import { renderPrintJob } from './escpos-renderer';
...
/**
 * Execute a print job: render template → send to physical printer → ack back to server.
 */
export async function executePrintJob(job: PrintJob): Promise<PrintResult> {
    ...
    try {
        // 1. Render template to ESC/POS annotated text
        const text = renderPrintJob(job, CURRENCY_SYMBOL);
        console.log(`[PrintExecutor] Rendered job ${jobId} → ${text.length} chars for "${printer.name}"`);
        ...
        for (let i = 0; i < copies; i++) {
            ...
            if (printer.type === 'NETWORK') {
                result = await api.printerPrintTextTCP(
                    printer.address,
                    printer.port || 9100,
                    text
                );
            } else if (printer.type === 'USB') {
                result = await api.printerPrintTextUSB(
                    printer.address,
                    text
                );
            } else {
                result = { success: false, error: `Tipo de impresora no soportado: ${printer.type}` };
            }
...
/**
 * Quick print: render an order using a template and send to a specific printer.
 * Used for manual "print" button clicks (not from WebSocket jobs).
 */
